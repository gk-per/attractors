defmodule VaryingDadrasAttractor do
  ...
  def simulate(num_points, dt, d) do
    initial_state = {1.0, 1.0, 1.0}
    states = Stream.iterate(initial_state, &next_state(&1, dt, d))
    Enum.take(states, num_points)
  end

  defp next_state({x, y, z}, dt, d) do
    dx = y - @a * x + @b * y * z
    dy = @c * y - x * z + z
    dz = d * x * y - @e * z
  ...
  def generate_complex_c_sequence do
    up = generate_sequence(8.2, 8.26, 1.0e-4)
  ...
  def plot_frame(points, frame, c, total_frames) do
    ...
    set isosamples 100
    set hidden3d
    splot '/tmp/lorenz_data_#{frame}.dat' every ::1 with lines lc rgb '#FF5733' lw 2.5 notitle
    """
